...
function MapTileLayer({
  ...
  const DEFAULT_DARK_URL = 'https://tile.openstreetmap.org/{z}/{x}/{y}.png';
    ...
    if (tileLayers.length > 0 && !selectedTileLayer) {
      const validDefaultValue =
        ...
        tileLayers.some((layer) => layer.name === defaultTileLayer)
          ? defaultTileLayer
          : tileLayers[0]?.name;
      if (validDefaultValue) setSelectedTileLayer(validDefaultValue);
    }
...
function MapMarker({
  icon = <MapPinIcon className="size-6 fill-blue-600 stroke-white" />,
  iconAnchor = [12, 12],
  bgPos,
  popupAnchor,
  tooltipAnchor,

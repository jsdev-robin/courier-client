...
export const parcelAnalyticsApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    findStatsMetricsParcel: builder.query<FindStatsMetricsResponse, void>({
      query: () => ({
        url: `/analytics/agent/parcel/metrics/stats`,
        method: 'GET',
      }),
    ...
    >({
      query: () => ({
        url: `/analytics/agent/parcel/metrics/last7days`,
        method: 'GET',
      }),
    ...
    findMapMetricsParcel: builder.query<FindMapMetricsResponse, void>({
      query: () => ({
        url: `/analytics/agent/parcel/metrics/locations`,
        method: 'GET',
      }),
    ...
    >({
      query: () => ({
        url: `/analytics/agent/parcel/metrics/today/status/distribution`,
        method: 'GET',
      }),

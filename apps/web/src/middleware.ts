...
export default async function middleware(req: NextRequest) {
  ...
  const cookie = (await cookies()).get('xd92be3')?.value;
  const session = await utils.decrypt(cookie, process.env.REFRESH_TOKEN);

  if (isProtectedRoute && (!session || session.role !== 'user')) {
    await utils.deleteSession('xd91fe7');
    return NextResponse.redirect(new URL('/sign-in', req.nextUrl));
  }

  if (isPublicRoute && session && session.role === 'user') {
